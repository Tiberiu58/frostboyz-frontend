...
export default function Header() {
  ...
  // închidere la ESC + click dreapta
  ...
  return (
    <header className="flex items-center justify-between px-4 md:px-8 py-4 sticky top-0 bg-black/70 backdrop-blur z-50"
            onKeyDown={onKey}>
      <a href="/" className="text-cyan-300 tracking-widest font-extrabold uppercase">FrostBoyz</a>
      ...
      <nav className="hidden md:flex items-center gap-6 text-sm opacity-90">
        ...
        <a href="https://instagram.com" className="hover:text-cyan-300" target="_blank">Instagram</a>
      ...
      {open && (
        <>
          ...
          <aside className="fixed top-0 right-0 h-full w-[80%] max-w-[340px] bg-neutral-950 border-l border-cyan-300 p-6 flex flex-col gap-4">
            <button className="self-end border px-3 py-1 rounded-lg hover:bg-cyan-300 hover:text-black"
                    onClick={close}>Close</button>
            <a className="py-2 border-b border-neutral-800" href="#drops" onClick={close}>Drops</a>
            <a className="py-2 border-b border-neutral-800" href="https://instagram.com" target="_blank" onClick={close}>Instagram</a>
